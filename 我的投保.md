...
#### 基本信息
    请求类型：HTTP
    请求地址：http://127.0.0.1:8080/policys/listApplyPolicys
    请求方式：get
...
#### 基本信息  
     请求类型：HTTP
     请求地址：http://127.0.0.1:8080/policys/listApplyPolicysContinue
     请求方式：get
     请求类型：String ， int，int ,String
     响应类型：R
#### 接口描述  
...
#### 基本信息
     请求类型：HTTP
     请求地址：http://127.0.0.1:8080/policys/viewApplyPolicy
     请求方式：get
     请求类型：String ,String
     响应类型：R
#### 接口描述
...
```sql
   ...
   select gu.business_no,gc.corp_name appli_name,gc.uniform_code,gc.province,gc.city,gc.county,gc.town,gc.village,gc.address,gc.identity_no,gs.concat,gs.post_code,go.organ_name,go.identity_no insured_identity_no,gs.contact,go.mobile insured_mobile,gs.address,go.organ_name,gp.product_name risk_name,gu.amount,(select organ_name from ggorgan where organ_id = gu.insurance_code) insurance_code,gu.letter_id
    ...
    根据主键letter_id查询
    ### 资证
    select card_name，gu.corp_name ,card_type，card_no，end_date，card_img from ggmanagercard gm ，ggusercorp gu where gm.user_id = gu.user_id and gm.valid_status = '1'
    ### 合同表
...
#### 基本信息
请求类型： http
请求地址： http://ip/organs/listOrgans
请求方式： get
参数类型： String，String
响应类型： R
...
#### 基本信息
请求类型： http
请求地址： http://ip/products/listProducts
请求方式： get
参数类型： String ，String
响应类型： R
#### 接口描述
...
#### 基本信息
请求类型： http
请求地址： http://ip/products/getProductDetail
请求方式： get
参数类型： String ，String
响应类型： R
#### 接口描述
...
#### 基本信息
请求类型： http  
请求地址： http://ip/policys/getSavedData  
请求方式： get  
参数类型： String ，String  
响应类型： R  
#### 接口描述
...
#### 基本信息
请求类型： http
请求地址： http://ip/policys/savePolicy
请求方式： post
